      ...
      <section id="methods" className="section">
        ...
        <div className="case-grid rich-cases">
          ...
          <article className="case-card">
            ...
            <h3>同样到达目标，动作可以让人更早看懂。</h3>
            <p className="case-terms">
              意图可读性 Legibility · 可预测性 Predictability
            </p>
            <p>
              Dragan
              等区分了两个容易混在一起的问题。可预测性是：已经知道目标，动作是否符合我的预期？意图可读性是：只看到动作的一部分，我能否猜出目标？论文为两者建立形式化描述，并用视频实验研究人的判断。
              <Cite ids={[12]} />
            </p>
            <div className="concept-pair">
              <div>
                <span>可预测 / Predictable</span>
                <strong>知道目标 → 预期路径</strong>
              </div>
              <div>
                <span>意图可读 / Legible</span>
                <strong>看到路径 → 推断目标</strong>
              </div>
            </div>
            <p>
              实验中，一类任务让人表达预期轨迹或评价匹配程度；另一类让人观看动作视频，在认为自己知道目标时停止并猜测。这样就能比较“多早猜到”与“猜得对不对”。设计成更可读的轨迹能够支持更早的正确目标判断；机器人外观也会影响人原本期待的路径。
              <Cite ids={[12]} />
            </p>
            <p className="reading-location">
              原文怎么读：概念定义 → 图 6 的目标推断 → 图 7
              的预期差异。研究条件是视频观察，做共处原型时可据此再设计现场测试。
            </p>
            <MarginNote name="expression">
              如果你正在设计一个会移动、转向或展开的物件，它的哪一小段动作能提前“透露”目标？这个提示也许就藏在形态变化里。
            </MarginNote>
          </article>
          <article className="case-card">
            <div className="case-meta">
            ...
            <p>
              Koh
              等让模型先预测可理解的概念，再预测最终结果。论文研究了鸟类识别与膝关节
              X
              光评级。例如，鸟类模型可以先识别颜色、形态等属性，再根据这些概念判断物种。这样的中间层，让解释变成一个可以介入的位置。
              <Cite ids={[10]} />
            </p>
            <div className="method-flow">
              图像 Image → 可检查的属性 Concepts → 类别 Prediction
              <br />
              修正属性 ↗ 再看预测怎样变化
            </div>
            <p>
              论文的图 3 展示了通过修正概念改变预测的例子；图 4
              比较不同训练方式下的干预效果。在鸟类实验中，作者利用真实概念标签模拟纠正，并按概念组、可见性处理干预。这使我们能研究模型是否“改得动”，但还没有直接回答普通使用者会不会操作、愿不愿意纠正。
              <Cite ids={[10]} />
            </p>
            <p className="case-design">
              <b>设计延伸。</b>
              可以把“概念检查”做成一个轻量协作环节：指向证据、修正属性、查看结果变化。值得设计的是这三步如何让用户理解自己的影响。
            </p>
