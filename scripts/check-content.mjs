...
const sourceUrl='data:text/javascript;base64,'+Buffer.from(sourcesModule).toString('base64');
const glossaryModule=ts.transpile(read('app/glossary-data.ts'),{module:ts.ModuleKind.ESNext}).replace(/(['"])\.\/sources\1/g,JSON.stringify(sourceUrl));
const {allTerms,glossaryGroups,glossarySources}=await import('data:text/javascript;base64,'+Buffer.from(glossaryModule).toString('base64'));
...
allTerms.forEach(term=>{
  ...
  term.related.forEach(id=>assert(termIds.has(id),term.id+' links to missing term '+id));
});
for(const path of ['app/field-guide.tsx','app/learning-content.tsx']){
  for(const match of read(path).matchAll(/href="\/glossary#([^"]+)"/g))assert(termIds.has(match[1]),'Missing linked term '+match[1]);
}
for(const id of ['cases','studio','sources'])assert(content.includes('id="'+id+'"'),'Legacy anchor missing '+id);
assert.equal((content.match(/<section /g)||[]).length,3,'Main guide should have three chapters here plus mapping');
console.log('Glossary checks passed: 138 bilingual entries in 8 groups, '+new Set(allTerms.map(t=>t.source)).size+' definition sources, all related links valid; four main chapters and legacy anchors retained.');
